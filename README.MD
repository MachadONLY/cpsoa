...
- **Cadastro de Usuários**: Permite o cadastro de novos usuários com senha encriptada.
- **Autenticação**: Sistema de autenticação via JWT (já existente no projeto base).
- **Controle de Acesso (Roles)**: Implementação de roles (ex: ADMIN, USER) para controle de acesso.
- **Gerenciamento de Usuários (ADMIN)**: Apenas administradores podem cadastrar, listar, atualizar e excluir usuários.
- **Alteração de Senha**: Usuários autenticados podem alterar suas próprias senhas.
...
### Usuários
- `POST /api/users/register` - Cadastrar novo usuário (acesso público)
- `GET /api/users` - Listar todos os usuários (apenas ADMIN)
- `PUT /api/users/{id}` - Atualizar perfil de usuário (apenas ADMIN)
- `DELETE /api/users/{id}` - Excluir usuário (apenas ADMIN)
- `PUT /api/users/{id}/change-password` - Alterar senha do usuário (usuário autenticado)
...
- Foi adicionada uma estrutura básica de segurança com Spring Security, incluindo autenticação e autorização baseada em roles. É necessário integrar com o sistema de autenticação JWT existente para um controle de acesso completo.
